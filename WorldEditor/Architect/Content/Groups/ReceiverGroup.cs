...
public class ReceiverGroup([CanBeNull] ReceiverGroup parent, params string[] types)
{
    ...
    internal static ReceiverGroup Ghosts;
    ...
    internal static void Initialize()
    {
        ...
        ObjectMover = new ReceiverGroup(Invisible,
            EventManager.RegisterEventReceiverType("move", o => { o.GetComponent<ObjectMover>().DoMove(); }));

        ObjectDuplicator = new ReceiverGroup(Invisible,
            EventManager.RegisterEventReceiverType("duplicate",
                o => { o.GetComponent<ObjectDuplicator>().Duplicate(); }));
	...
	Ghosts = new ReceiverGroup(Generic, EventManager.RegisterEventReceiverType("die", o =>
        {
            if (!o.activeInHierarchy) return;
            o.GetComponent<HealthManager>().Die(null, AttackTypes.Generic, true);
        }));
        ...
        TeleportPoint = new ReceiverGroup(Invisible,
            EventManager.RegisterEventReceiverType("teleport",
                o => { HeroController.instance.transform.position = o.transform.position; }));

        HazardRespawnPoint = new ReceiverGroup(Invisible,
            EventManager.RegisterEventReceiverType("setspawn",
                o => { PlayerData.instance.SetHazardRespawn(o.GetComponent<HazardRespawnMarker>()); }));
        ...
        Stompers = new ReceiverGroup(Invisible,
            ...
            EventManager.RegisterEventReceiverType("stopinstant",
                o => { o.GetComponentInChildren<StopAnimatorsAtPoint>().stopInstantEvent.ReceiveEvent(); }),
            EventManager.RegisterEventReceiverType("stop",
                o => { o.GetComponentInChildren<StopAnimatorsAtPoint>().stopEvent.ReceiveEvent(); }));
        ...
        Playable = new ReceiverGroup(Invisible,
            EventManager.RegisterEventReceiverType("play", o => { o.GetComponent<Playable>().Play(); })
        );

        Interactions = new ReceiverGroup(Invisible,
            EventManager.RegisterEventReceiverType("show_label", o => { o.GetComponent<Interaction>().GoUp(); }),
            EventManager.RegisterEventReceiverType("hide_label", o => { o.GetComponent<Interaction>().GoDown(); })
        );

        WalkTarget = new ReceiverGroup(Invisible,
            EventManager.RegisterEventReceiverType("walk", o => { o.GetComponent<WalkTarget>().DoWalk(); })
        );
        ...
        Mov = new ReceiverGroup(Playable,
            EventManager.RegisterEventReceiverType("pause", o => { o.GetComponent<MovObject>().Pause(); })
        );
        ...
        ColosseumPlatform = new ReceiverGroup(UpDownPlatforms,
            EventManager.RegisterEventReceiverType("down_slow",
                o => { o.LocateMyFSM("Control").SendEvent("SLOW RETRACT"); })
        );
        ...
        PlayerHook = new ReceiverGroup(Invisible,
            EventManager.RegisterEventReceiverType("kill_player",
                o => { HeroController.instance.TakeDamage(o, CollisionSide.other, 999, 2); }),
            EventManager.RegisterEventReceiverType("damage_player",
                o => { HeroController.instance.TakeDamage(o, CollisionSide.other, 1, 1); }),
            EventManager.RegisterEventReceiverType("hazard_player",
                o => { HeroController.instance.TakeDamage(o, CollisionSide.other, 1, 2); }),
            EventManager.RegisterEventReceiverType("heal_player", _ => { HeroController.instance.AddHealth(1); }),
            EventManager.RegisterEventReceiverType("full_heal_player", _ => { HeroController.instance.MaxHealth(); }),
            EventManager.RegisterEventReceiverType("gain_soul", _ => { HeroController.instance.SoulGain(); }),
            EventManager.RegisterEventReceiverType("max_soul", _ => { HeroController.instance.AddMPCharge(999); })
        );

        TextDisplay = new ReceiverGroup(Invisible,
            EventManager.RegisterEventReceiverType("display", o => { o.GetComponent<TextDisplay>().Display(); })
        );
    }
}